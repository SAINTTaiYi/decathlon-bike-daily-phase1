...
- 在 Preview 环境访问真实 Shiphub（Preview 固定 fixture）。
- 以明文保存 Shiphub 账号密码（v1.4 起改为 AES-256-GCM 信封加密存 CF Secret，见 8.1）。
...
浏览器永不获得 Shiphub access token 或 refresh token，也不直接调用 Shiphub API。v1.4 起「连接 Shiphub」由 Worker 使用加密存储的门店账号在服务端自动完成 IdP 表单登录（见 8.1），浏览器不参与登录过程。
...
**硬规则（用户强制，v1.4）**：任何对 Shiphub 上游的调用（定时、手动、授权触发）只允许发生在**北京时间 10:00–22:00**（固定 `Asia/Shanghai`，不随门店时区字段或部署环境变化）。营业时间外：定时任务跳过、手动同步返回 `403 SHIPHUB_OUTSIDE_BUSINESS_HOURS`、同步引擎层还有第二道兜底校验。该窗口通过 `SHIPHUB_ACTIVE_START_HOUR` / `SHIPHUB_ACTIVE_END_HOUR` 注入部署配置，代码默认值同为 10/22。
...
### 8.1 设置中心连接模型（v1.4：一键自动重连）

设置中心只提供连接状态和操作，不提供 Shiphub 账密输入表单：

1. 用户点击「连接 Shiphub」，Worker 检查是否配置了加密门店账号凭据（CF Secret）。
2. **已配置（默认路径，v1.4）**：Worker 在服务端自动完成全部流程——生成一次性 state/PKCE verifier → 请求 IdP 授权页并解析登录表单 → 提交 `pf.username`/`pf.pass`（携带会话 cookie）→ 从跳转 URL 捕获 authorization code（校验 state）→ 携带 `code_verifier` 交换 token。全程无浏览器跳转，手机同样适用。
3. **未配置（兜底路径）**：沿用浏览器 SSO 跳转流程（受 IdP 回跳地址注册限制，见 8.2 约束）。
4. 交换成功后 refresh token 信封加密写入当前 `store_id` 的连接记录，并立即触发后台同步（`waitUntil`）。
5. 设置中心只显示 `connected`、`reauth_required`、最后同步时间和脱敏错误码，不显示 token、账号密码或完整外部门店标识。
6. 用户点击断开时删除本地 refresh token 并清理连接状态；不得提供密码找回或密码导出。

**账密加密存储（v1.4 实现方式）**：门店账号用户名与密码分别以 AES-256-GCM 信封加密（随机 12 字节 nonce，密文与密钥分离），三个值独立存放于 Cloudflare Secret：

```text
SHIPHUB_LOGIN_KEY=<secret：32 字节 base64url 密钥>
SHIPHUB_LOGIN_USERNAME_ENC=<secret：密文.nonce>
SHIPHUB_LOGIN_PASSWORD_ENC=<secret：密文.nonce>
```

- 密钥与密文分开存放；仅登录瞬间在 Worker 内存解密，解密结果不落日志、不落审计、不落业务库、不进入 API 响应。
- 密码用于向 IdP 登录表单提交（等同账号持有人本人登录），不用于任何其他用途。
- 若未来改为独立 Credential Broker/HSM，主网站 Worker 与 D1 不得持有明文密码，本方案当前由信封加密 + Secret 隔离提供同等纵深。
...
SHIPHUB_LOGIN_KEY=<secret：门店账号加密密钥>
SHIPHUB_LOGIN_USERNAME_ENC=<secret：密文.nonce>
SHIPHUB_LOGIN_PASSWORD_ENC=<secret：密文.nonce>
...
1. 访问授权端点，IdP 返回 PingFederate 登录表单（表单 action 为 `/as/{token}/resume/...`，隐藏字段 `pf.adapterId`/`pf.ok`/`pf.cancel`/`pf.passwordreset`）。
2. 提交用户名密码（`pf.username` / `pf.pass`）到表单 action（**必须携带第一步下发的会话 cookie，否则 IdP 返回 Page Expired**）；校验通过后 302 重定向到 `redirect_uri` 并携带一次性 `authorization code`。
3. 服务端用客户端凭证（`Authorization: Basic base64(client_id:client_secret)`）交换 code——**必须同时携带 `code_verifier`**（授权请求发送了 PKCE S256 challenge，缺失时 IdP 返回 400 `code_verifier mismatch`；2026-08-19 线上事故根因），获得 `access_token`（约 2 小时）与 `refresh_token`。
4. `refresh_token` 每次刷新轮换（旧 token 立即失效）；信封加密存入 D1，持续自动刷新，正常运营无需重新登录。
5. 首次部署可通过 bootstrap `refresh_token`（Cloudflare Secret 注入）初始化连接，之后由 D1 托管轮换；v1.4 起首选「门店账号程序化登录」自动重连，bootstrap 作为兜底。
6. 若令牌长期失效需要重新授权：v1.4 起在 ops 点击「连接 Shiphub」即可自动恢复（见 8.1）。

**IdP 侧硬约束（已实测确认）**：

- 该 OAuth client 仅注册了**一个**回跳地址（官方站点根路径，无末尾斜杠）。workshop.skin 的回调地址未注册，浏览器 SSO 跳转的 code 只会落在官方站点，无法自动回到 Workshop——这是「手动抓 code」与「自动重连」两条路径的根本原因。
- **重新登录会使所有旧 refresh token 失效**（多次实测踩坑）：任何一次新的 IdP 登录都会作废此前抓取的 refresh token，抓取 token 后不得再触发新的登录。
- 门店账号程序化登录（表单自动提交）经授权与连通性验证，无验证码/风控拦截；云出口 IP 同样可用。
