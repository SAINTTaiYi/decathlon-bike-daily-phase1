...
test('维修消散以完整白色像素格向左离场，且 reduced motion 直接提交最终状态', () => {
  ...
  assert.match(ledger, /const departure = pixels\.map/)
  assert.match(ledger, /rightFirstBias/)
  assert.match(ledger, /\.set\(pixel, \{ x: -jump, y: lane, scale: 1 \}, at\)/)
  assert.match(ledger, /\.set\(pixel, \{ autoAlpha: 0 \}, at \+ \.075\)/)
  assert.match(ledger, /data-repair-pixel-dissolving/)
  assert.match(ledger, /onRepairPixelDissolveComplete/)
  assert.match(styles, /V5\.6\.9 repair completion/)
  assert.match(styles, /background: var\(--ink\)/)
  assert.match(styles, /box-shadow: inset 0 0 0 1px rgb\(11 11 12 \/ \.56\)/)
  assert.match(styles, /background: var\(--ink\)/)
