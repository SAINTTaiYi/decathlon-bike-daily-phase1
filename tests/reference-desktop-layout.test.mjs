...
test('1536x1024 board scales as a single physical-pixel canvas on desktop-mode tablets', () => {
  ...
  assert.match(css, /--ops-desktop-workbench-fit: min\(calc\(100vw \/ 1536px\), calc\(100dvh \/ 1024px\)\)/u)
  assert.match(css, /align-items: safe center/u)
  assert.match(css, /justify-items: safe center/u)
  assert.match(css, /zoom: var\(--ops-desktop-workbench-fit\)/u)
  assert.match(app, /className="desktop-workbench-fit"/u)
