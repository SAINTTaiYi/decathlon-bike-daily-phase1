...
test('桌面与平板使用同一 Workshop 主题并按断点重排，不加载第二套 Endfield 主题', async () => {
  ...
  assert.match(css, /Normal vertical module flow/u)
  assert.match(css, /\.workshop-module-stack \{[^}]*flex-direction: column;/su)
  assert.doesNotMatch(css, /\.workshop-module-stack \{[^}]*grid-template-columns:/su)
