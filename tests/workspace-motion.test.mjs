...
test('one passive requestAnimationFrame runtime derives active scene from native scrolling', () => {
  ...
  assert.match(hook, /viewportHeight \* 0\.62/u)
  assert.match(hook, /viewportHeight \* 0\.82/u)
...
test('the full-page S trajectory and Overview mineral share bounded scroll variables', () => {
  ...
  assert.match(reconstruction, /assembly-stage-image \{ position: absolute; inset: 0; z-index: 0/u)
  assert.match(reconstruction, /assembly-stage-field[\s\S]*linear-gradient/u)
...
test('full reconstruction includes four-band load transition and five distinct sticky material stages', () => {
  ...
  assert.match(reconstruction, /position: sticky;\s*\n\s*top: 0/u)
  assert.match(reconstruction, /data-assembly-stage='places'/u)
  assert.match(reconstruction, /width: 100%;\n  max-width: none;/u)
  assert.match(reconstruction, /record-ledger[\s\S]*grid-template-columns: repeat\(auto-fit/u)
  assert.match(reconstruction, /background: linear-gradient\(180deg/u)
  assert.doesNotMatch(reconstruction, /inset: 8% 6% 10%/u)
})

test('active-scene changes cannot reflow the shell at the pickup boundary', () => {
  const reconstruction = readFileSync(new URL('../apps/web/src/styles/assembly-reconstruction.css', import.meta.url), 'utf8')
  assert.match(reconstruction, /workshop-runtime \.workshop-shell,\s*\n\.workshop-runtime\[data-active-scene='pulse'\] \.workshop-shell \{[^}]*width: 100%;[^}]*max-width: none;[^}]*min-height: 100dvh;[^}]*padding: 0/u)
  assert.match(reconstruction, /workshop-runtime > \[data-workspace-layer='navigation'\] \{[\s\S]*position: fixed !important;[\s\S]*background: linear-gradient/u)
})
