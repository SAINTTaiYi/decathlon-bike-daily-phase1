...
test('Worker：GET 登录态可读，手动同步需 CSRF + 管理员；迁移与 schema 版本对齐', async () => {
  ...
  assert.match(sync, /BI_SYNC_TIMEZONE = 'Asia\/Shanghai'/u, '当日守卫固定北京时间（与 Shiphub 硬规则一致）')
  assert.match(sync, /reason: 'FRESH_TODAY'/u, '当日已同步过即跳过')
  assert.match(sync, /runLoginBiSkuSync/u, '登录触发入口不抛错（记日志）')
  ...
  assert.match(schema, /'0034_shiphub_sync_runs_started_index'/u)
  ...
  assert.doesNotMatch(index, /BiSkuSync/u, 'cron（*/5）不得再挂 BI 同步')
  const auth = await read('apps/worker/src/routes/auth.ts')
  assert.match(auth, /waitUntil\(runLoginBiSkuSync\(c\.env\)\)/u, '登录成功后触发当日 BI 同步')
})
