...
test('Add Pickup and the entire expanded card share one opaque action yellow', () => {
  assert.doesNotMatch(css, /--pickup-action-yellow/)
  assert.match(css, /pickup-ledger-global-actions button:last-child,\s*\n\.pickup-ledger \.pickup-card-frame\[data-expanded='true'\] \.pickup-card \{[^}]*background-color: var\(--ops-yellow\)[^}]*background-image: none[^}]*opacity: 1/s)
  assert.match(css, /pickup-card-frame\[data-expanded='true'\] \.pickup-card-(?:summary|detail)[^{]*[\s\S]*background: transparent/)
  assert.match(css, /0 0 22px var\(--ops-yellow-glow\), 0 7px 18px rgb\(64 55 34 \/ \.1\)/)
})
...
test('every expanded Pickup and repair card has the Add Pickup-style outer halo', () => {
  ...
  assert.match(css, /\.pickup-ledger \.pickup-card\[data-expanded='true'\] \{[\s\S]*background: var\(--ops-yellow[\s\S]*box-shadow: none/u)
