...
test('工作台入场期间可延迟版本弹窗，避免抢占跳过动画的焦点', () => {
  ...
  assert.match(appSource, /workspaceEntryEligible = \['login', 'restore', 'registration'\]\.includes\(auth\.source\)/u)
  assert.match(appSource, /deferUpdatePrompt = workspaceEntryEligible/ )
  assert.match(appSource, /window\.scrollY <= 8/)
  assert.match(appSource, /#module-\(pulse\|pickup\|poster\|repair\|resale\|sales\)/)
