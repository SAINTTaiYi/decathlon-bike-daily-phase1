...
- Motion is short and functional. The six operational modules may use a brief native sticky cover/reveal stage before releasing into their normal document flow; stage navigation must use stable anchors and passive observation only, never wheel/touch interception, pinned spacers, or content-size-driven scroll correction. Respect `prefers-reduced-motion`; never require animation to understand state.
