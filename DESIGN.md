...
- V5.7.8 首屏仅以“闭店档案封面”组织：刊头、身份条、闭店摘要、唯一下一动作与当前版本说明收在 `workspace-first-screen` 内。该封面允许使用极低对比抽象工程坐标/圆规弧线和最多两个非语义荧光黄定位点；不得使用人物、自行车或任何具象插画。工坊图片和后续 KPI、台账模块继续使用原有安静的运营布局，不扩散封面构图。
- V5.7.10 待取区在移动端与桌面端同步采用用户提供参考图的技术档案式排版：`02 PICKUP` 编号、超大 `PICKUP BOARD` 标题、中文说明、待取计数分隔、档案台账刊头、状态胶囊和左右并列的编辑/确认取车操作。只改待取内容区；全局 Dock、真实数据、通知切换、取货校验、权限、审计、左滑删除与确认取车像素完成效果保持原样。浏览器 chrome 与人体插图明确排除，以 CSS 工程圆弧、标尺、结构线、纸张纹理和稀疏荧光黄信号点替代。
