...
export async function hashPassword(password: string, pepper: string): Promise<string> {
  ...
  const bits = await crypto.subtle.deriveBits({ name: 'PBKDF2', salt, iterations: PASSWORD_PBKDF2_ITERATIONS, hash: 'SHA-256' }, keyMaterial, 256)
  const saltB64 = btoa(String.fromCharCode(...salt)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/u, '')
  return `pbkdf2$sha256$${PASSWORD_PBKDF2_ITERATIONS}$${saltB64}$${toHex(bits)}`
}
...
export async function verifyPassword(passwordHash: string, password: string, pepper: string): Promise<boolean> {
  ...
  if (!Number.isInteger(iterations) || iterations !== PASSWORD_PBKDF2_ITERATIONS) return false
