...
export function buildRestoreSnapshotStatements(db: D1Database, snapshot: Record<string, unknown>): D1PreparedStatement[] {
  ...
  const statements: D1PreparedStatement[] = [
    ...
    repair
      ...
      : db.prepare('DELETE FROM repair_details WHERE work_item_id = ?').bind(id),
    pickup
      ? db.prepare(`
      ...
      : db.prepare('DELETE FROM pickup_details WHERE work_item_id = ?').bind(id),
    resale
      ? db.prepare(`
      ...
      : db.prepare('DELETE FROM resale_details WHERE work_item_id = ?').bind(id),
    handover
      ? db.prepare(`
      ...
      : db.prepare('DELETE FROM handover_details WHERE work_item_id = ?').bind(id)
  ]
...
export async function restoreSnapshot(db: D1Database, snapshot: Record<string, unknown>): Promise<void> {
  await db.batch(buildRestoreSnapshotStatements(db, snapshot))
}
