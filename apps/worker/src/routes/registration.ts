...
export function registrationRoutes() {
  ...
  app.post('/api/v1/registration/otp', requireJsonBody, async (c) => {
    ...
    // 加入已有门店：命中已生效门店（active 且非自注册占位）时不再 409，
    // 而是走「验证邮箱 → 创建无门店账号 + 待审批加入申请」路径。
    // storeName 对加入分支无意义（真实店名以库内为准），complete 时回显。
    const joinTarget = existingStore && existingStore.status === 'active' && existingStore.self_registration_pending === 0
      ? existingStore
      : null
