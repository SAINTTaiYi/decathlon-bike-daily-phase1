...
const IDLE = { status: 'idle', newBikes: null, usedBikes: null, newTo: null, usedTo: null, safety: null, storeDay: null, syncedAt: null }
export default function useBikeDaySync(open, { enabled = true } = {}) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setState({ status: 'syncing', newBikes: null, usedBikes: null, newTo: null, usedTo: null, safety: null, storeDay: null, syncedAt: null })
    inflight.current = getBikeDay()
      .then((payload) => {
        if (cancelled) return
        if (!payload || payload.available !== true) {
          setState({ status: 'unavailable', newBikes: null, usedBikes: null, newTo: null, usedTo: null, safety: null, storeDay: null, syncedAt: null })
          return
        }
        setState({
          status: 'ok',
          ...
          storeDay: payload.storeDay ?? null,
          syncedAt: payload.syncedAt ?? null
        })
      })
      .catch(() => {
        if (!cancelled) setState({ status: 'error', newBikes: null, usedBikes: null, newTo: null, usedTo: null, safety: null, storeDay: null, syncedAt: null })
      })
