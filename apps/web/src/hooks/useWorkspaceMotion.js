...
export default function useWorkspaceMotion({ active, rootRef, onComplete }) {
  ...
  useLayoutEffect(() => {
    ...
    const timeline = gsap.timeline({ defaults: { ease: 'expo.out' }, onComplete: finish })
      ...
      .fromTo(environment, { autoAlpha: 0 }, { autoAlpha: 1, duration: .26 }, 0)
      .fromTo(navigation, { autoAlpha: 0, y: -8 }, { autoAlpha: 1, y: 0, duration: .34, stagger: .035 }, .06)
      .fromTo(structure, { autoAlpha: 0, y: 14 }, { autoAlpha: 1, y: 0, duration: .44 }, .12)
      .fromTo(module, { autoAlpha: 0, y: 12 }, { autoAlpha: 1, y: 0, duration: .4 }, .16)
      .fromTo(dock, { autoAlpha: 0, y: 10 }, { autoAlpha: 1, y: 0, duration: .32 }, .24)
