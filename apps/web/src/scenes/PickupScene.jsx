...
export default function PickupScene(props) {
  ...
  return (
    <section className="look-section pickup-look" data-depth-section={scene.id} id={scene.id} data-look={scene.id} aria-labelledby={`${scene.id}-title`}>
      <SceneTitle scene={scene} note="票据标明来源并突出维修取车日期；点按通知状态可选择“已通知”。自提需核对取货码；维修完成状态按开单类型放行，质保付款取车前提醒过机核验。" />
