...
export default function SalesScene({ kpi, kpiReady, savedAt, closedAt, onEditKpi, onHistory, storeCode = '' }) {
  ...
  return (
    <section className="look-section closing-look" data-depth-section={scene.id} id={scene.id} data-look={scene.id} aria-labelledby={`${scene.id}-title`}>
      ...
      <div className="sales-bi-slot" data-motion="data">
        {viewport === 'desktop' ? <BiInsightPanel storeCode={storeCode} /> : <BiSalesMobile storeCode={storeCode} />}
      </div>
    </section>
  )
}
