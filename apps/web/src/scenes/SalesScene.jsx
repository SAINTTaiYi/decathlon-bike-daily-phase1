...
export default function SalesScene({ kpi, kpiReady, savedAt, closedAt, onEditKpi, onHistory }) {
  ...
  return (
    <section className="look-section closing-look" id={scene.id} data-look={scene.id} aria-labelledby={`${scene.id}-title`}>
      <SceneTitle scene={scene} note="每天由同事人工填写销售车辆、安全检查、评价、二手售出与收车数据；保存后即可完成闭店。" action={titleAction} />
      <div className="sales-input-summary" data-ready={kpiReady ? 'true' : 'false'}>
