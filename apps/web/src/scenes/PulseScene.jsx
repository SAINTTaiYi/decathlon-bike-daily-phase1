...
export default function PulseScene({ dateKey, kpi, kpiReady, records, closedAt, onJump, onEditKpi, onHistory }) {
  ...
  return (
    <section className="look-section kpi-look" id={scene.id} data-look={scene.id} aria-labelledby={`${scene.id}-title`}>
      <SceneTitle scene={scene} note="销售数据是唯一闭店要求。维修、待取、二手车和其它交接只在实际变化时编辑，未操作事项会跨日延续。" action={titleAction} />
      <div className="kpi-sheet" aria-label="当日 workshop KPI">
      ...
      <nav className="handover-index" aria-label="业务台账模块">
