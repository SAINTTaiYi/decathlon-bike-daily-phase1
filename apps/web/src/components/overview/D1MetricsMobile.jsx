...
/* ── G18 · 今日写行大数字 + 刻度进度条（移动版）────────────── */
export function D1MobileWriteUsage({ snapshot, stale }) {
  ...
  const build = useMemo(() => (timeline, node) => {
    const state = { value: 0 }
    timeline.to(state, { value: totals.rowsWritten, duration: 1.0, ease: 'expo.out', onUpdate: () => {
      const el = node.querySelector('[data-d1mw-counter]')
      if (el) el.textContent = fmtRows(Math.round(state.value))
    } }, 0.15)
    timeline.from(node.querySelectorAll('[data-d1mw-chip]'), { opacity: 0, y: 6, duration: 0.5, ease: 'power3.out', stagger: 0.07 }, 0.5)
    timeline.from(node.querySelectorAll('[data-d1mw-bar-tick]'), { opacity: 0, duration: 0.3, stagger: 0.02 }, 0.25)
    timeline.fromTo(node.querySelector('[data-d1mw-bar-fill]'), { scaleX: 0 }, { scaleX: 1, duration: 0.9, ease: 'expo.out' }, 0.3)
    timeline.from(node.querySelector('[data-d1mw-bar-mark]'), { opacity: 0, duration: 0.4 }, 0.95)
  }, [totals.rowsWritten])
  useD1Motion(ref, revealed, replay, reduced, build)
  return (
    <section ref={ref} className="d1-mm-card d1-mm-usage" data-replay={replay} onClick={replayChart} aria-label={`当日 D1 写行 ${fmtRows(totals.rowsWritten)} 行，占免费限额 ${usedPct}%`}>
      <h3>D1 写行 · 今日已用 {usedPct}%</h3>
      <div className="d1-mm-sub">免费限额 10 万行/日 · 北京 08:00 归零{stale ? ' · 同步失败，显示最近数据' : ''}</div>
      <div className="d1-mm-usage-main" data-d1mw-chip="">
        <b data-d1mw-counter="">{fmtRows(totals.rowsWritten)}</b>
        <span className="d1-mm-usage-chip">{projectedPct}%</span>
        <small>预计全天</small>
      </div>
      <div className="d1-mm-usage-bar" role="img" aria-label={`写入用量进度：当前 ${usedPct}%，预计全天 ${projectedPct}%`}>
        <i className="d1-mm-bar-track" />
        <i className="d1-mm-bar-fill" data-d1mw-bar-fill="" style={{ '--d1-bar-scale': usedPct / 100 }} />
        {[10, 20, 30, 40, 50, 60, 70, 80, 90].map((mark) => <span key={mark} className="d1-mm-bar-tick" data-d1mw-bar-tick="" style={{ left: `${mark}%` }} />)}
        <em className="d1-mm-bar-mark" data-d1mw-bar-mark="" style={{ left: `${projectedPct}%` }} title={`预计全天 ${fmtRows(projectedWritesFullDay)} 行`} />
      </div>
      <div className="d1-mm-db-strip" data-d1mw-chip="">
        {databases.map(({ database, rowsWritten }) => (
          <div key={database} className="d1-mm-db-item"><small>{database === 'staging' ? '正式库' : '预览库'}</small><b>{fmtRows(rowsWritten)}</b></div>
        ))}
      </div>
    </section>
  )
...
/* ── B2 发丝线 · 逐小时写行（移动压高版）──────────────────── */
export function D1MobileWriteHourly({ snapshot }) {
  const { series, totals } = snapshot
  const { ref, revealed, replay, replayChart } = useD1Reveal()
  const reduced = usePrefersReducedMotion()
  const geometry = useMemo(() => {
  ...
  const build = useMemo(() => (timeline, node) => {
    const line = node.querySelector('[data-d1mw-line]')
    if (line) {
      const length = line.getTotalLength?.() ?? 300
      timeline.fromTo(line, { strokeDasharray: length, strokeDashoffset: length }, { strokeDashoffset: 0, duration: 1.0, ease: 'power2.inOut' }, 0.15)
    }
    timeline.from(node.querySelectorAll('[data-d1mw-dot]'), { scale: 0, duration: 0.4, ease: 'back.out(1.6)', stagger: 0.014 }, 0.5)
    timeline.from(node.querySelectorAll('[data-d1mw-ax]'), { opacity: 0, duration: 0.4, ease: 'power2.out' }, 0.8)
  }, [geometry])
  useD1Motion(ref, revealed, replay, reduced, build)
  const visible = [0, 6, 12, 18, 23]
  return (
    <section ref={ref} className="d1-mm-card d1-mm-hourly" data-replay={replay} onClick={replayChart} aria-label={`逐小时写行，累计 ${fmtRows(totals.rowsWritten)} 行`}>
      <h3>逐小时写行 · 峰值 {peak ? `北京 ${String((peak.hour + 8) % 24).padStart(2, '0')}:00` : '—'}</h3>
      <div className="d1-mm-sub">1 点 = 1 个 UTC 小时 · 横轴括号为北京时间 · 点击重播</div>
      <svg className="d1-mm-chart d1-mm-chart-wide" viewBox="0 0 300 118" role="img" aria-label={`逐小时写行曲线，峰值 ${fmtRows(peak?.value ?? 0)} 行，累计 ${fmtRows(totals.rowsWritten)} 行`}>
        <title>点击重播入场动画</title>
        <line x1="14" y1={geometry.base} x2="286" y2={geometry.base} stroke={MONO.grid} strokeWidth="0.7" />
        <path data-d1mw-line="" d={`M ${geometry.path}`} fill="none" stroke={MONO.ink} strokeWidth="1" />
        {geometry.points.map((point) => {
          const hasData = series.some((entry) => entry.hour === point.hour)
          const isPeak = peak && point.hour === peak.hour && point.value > 0
          return <g key={point.hour}>
            <title>{`UTC ${String(point.hour).padStart(2, '0')}:00（北京 ${String((point.hour + 8) % 24).padStart(2, '0')}:00）：${fmtRows(point.value)} 行`}</title>
            <circle data-d1mw-dot="" cx={point.x} cy={point.y} r={isPeak ? 3.6 : 1.8} fill={hasData ? MONO.ink : '#F0EFEB'} stroke={MONO.ink} strokeWidth={hasData ? 0 : 0.8} />
            {isPeak ? <text x={point.x} y={point.y - 9} fontSize="9" fontWeight="800" fill={MONO.ink} textAnchor="middle" style={{ paintOrder: 'stroke', stroke: '#F6F4EE', strokeWidth: 3 }}>{fmtRows(point.value)}</text> : null}
          </g>
        })}
        {visible.map((hour) => <text key={hour} data-d1mw-ax="" x={geometry.x(hour)} y={geometry.base + 16} fontSize="7" fontWeight="600" fill={MONO.muted} textAnchor="middle" letterSpacing=".08em">{`${String(hour).padStart(2, '0')}(${String((hour + 8) % 24).padStart(2, '0')})`}</text>)}
      </svg>
    </section>
  )
}

/* ── C1 tick rows · 写行 Top 榜（移动紧凑行 + 进度条）────────── */
// 写行热点随当天活动变化，按 SQL 目标表名识别；识别不出时回退「写入 N」。
...
export function D1MobileWriteTop({ snapshot }) {
  const { topWrites, totals } = snapshot
  const { ref, revealed, replay, replayChart } = useD1Reveal()
  const reduced = usePrefersReducedMotion()
  const rows = useMemo(() => (topWrites ?? []).slice(0, 5).map((entry, index) => ({ ...entry, label: writeQueryLabel(entry.query, index), index })), [topWrites])
  const maxRows = Math.max(1, ...rows.map((row) => row.rowsWritten))
  const build = useMemo(() => (timeline, node) => {
    timeline.from(node.querySelectorAll('[data-d1mw-row]'), { opacity: 0, x: -8, duration: 0.35, stagger: 0.06, ease: 'power2.out' }, 0)
    node.querySelectorAll('[data-d1mw-bar-i]').forEach((bar) => {
      const timeline2 = timeline
      const target = bar
      timeline2.fromTo(target, { scaleX: 0 }, { scaleX: 1, duration: 0.7, ease: 'expo.out' }, 0.2)
    })
  ...
  useD1Motion(ref, revealed, replay, reduced, build)
  return (
    <section ref={ref} className="d1-mm-card d1-mm-top" data-replay={replay} onClick={replayChart} aria-label={`当日写行 Top 5 查询，首位 ${fmtRows(rows[0]?.rowsWritten ?? 0)} 行`}>
      <h3>写行 Top 5 · {rows[0] ? `${rows[0].label} ${fmtRows(rows[0].rowsWritten)}` : '暂无数据'}</h3>
      <div className="d1-mm-sub">今日共 {fmtRows(totals.rowsWritten)} 行 · 悬停看 SQL</div>
      <ol className="d1-mm-rows">
        {rows.map((row) => (
          <li key={row.index} data-d1mw-row="" title={`${row.label} — ${fmtRows(row.rowsWritten)} 行 × ${row.count} 次`}>
            <span className="d1-mm-rank">{String(row.index + 1).padStart(2, '0')}</span>
            <div className="d1-mm-rowmain">
              <div className="d1-mm-rowline"><span className="d1-mm-name">{row.label}</span><b>{fmtRows(row.rowsWritten)}</b><small>× {fmtRows(row.count)}</small></div>
              <div className="d1-mm-bar"><i data-d1mw-bar-i="" style={{ width: `${Math.max(3, (row.rowsWritten / maxRows) * 100)}%` }} /></div>
            </div>
          </li>
        ))}
      </ol>
...
export default function D1MetricsMobile({ snapshot, stale }) {
  ...
  return (
    <div className="d1-mm-panel" aria-label="D1 当日用量监控（读行与写行）">
      <D1MobileUsage snapshot={snapshot} stale={stale} />
      <D1MobileHourly snapshot={snapshot} />
      <D1MobileTop snapshot={snapshot} />
      <D1MobileWriteUsage snapshot={snapshot} stale={stale} />
      <D1MobileWriteHourly snapshot={snapshot} />
      <D1MobileWriteTop snapshot={snapshot} />
    </div>
  )
}
