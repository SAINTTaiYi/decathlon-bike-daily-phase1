...
const operationLinks = [
  { id: 'pickup', label: 'PICKUP' }, { id: 'poster', label: 'OTHER' }, { id: 'repair', label: 'REPAIR' }, { id: 'resale', label: 'USED' }, { id: 'sales', label: 'SALES' }
]
...
export default function WorkshopOverviewPage({ workflow, online, onEditKpi, onCompleteClosing, onHistory, onRefresh, onReopenClosing, onExportReport, onJump }) {
  ...
  const dashboardRows = [
    ...
    { label: '待处理工单', en: 'OPEN WORK ORDERS', value: metric(overviewRecords.length, available), accent: true },
  ...
  return <div className="ops-mobile-overview" data-workspace-module="true" aria-label="Workshop 业务总览">
    ...
    <main className="workshop-poster" aria-labelledby="workshop-poster-title">
      <section className="poster-opening">
        <div className="poster-registration" aria-hidden="true"><span>+</span><small>23°08' N<br />113°16' E</small></div>
        <header className="poster-title-block"><h1 id="workshop-poster-title"><span>WORKSHOP</span><span>OPS</span></h1><div className="poster-date"><small>DAY</small><strong>{date.monthDay}</strong><span>{date.weekday}</span><time dateTime={workflow.dateKey || undefined}>{date.full}</time></div></header>
        <div className="poster-system-line"><strong><i aria-hidden="true" />SYSTEM {online ? 'ONLINE' : 'OFFLINE'}</strong><span>FOCUS / EXECUTE / IMPROVE</span></div>
        <p className="poster-purpose"><strong>精益运营 · 高效协同</strong><span>让每一项工作都有价值</span></p>
        <span className="poster-arc" aria-hidden="true" /><span className="poster-crosshair poster-crosshair-a" aria-hidden="true">+</span><span className="poster-crosshair poster-crosshair-b" aria-hidden="true">+</span>
        ...
        <div className="poster-plinth" aria-hidden="true" /><p className="poster-object-note poster-object-note-left">WORK SMARTER<br />TOGETHER</p><p className="poster-object-note poster-object-note-right">DATA DRIVEN<br />RESULTS</p>
        <PosterClosingControl workflow={workflow} online={online} onEditKpi={onEditKpi} onCompleteClosing={onCompleteClosing} onHistory={onHistory} onRefresh={onRefresh} onReopenClosing={onReopenClosing} onExportReport={onExportReport} />
      </section>
      <section className="poster-kpi" aria-labelledby="poster-kpi-title"><div><small>PERFORMANCE / TODAY</small><h2 id="poster-kpi-title">Today KPI</h2><p>今日数据</p></div><button type="button" onClick={onEditKpi}><span>VIEW DASHBOARD</span><b aria-hidden="true">↗</b></button></section>
      <section className="poster-workzone" aria-labelledby="poster-overview-title">
        <button type="button" className="poster-photo poster-photo-left" onClick={() => onJump('repair')} aria-label="进入维修交接"><WorkbenchPicture className="poster-photo-media" position="18% center" /><span><small>01</small><strong>MAINTENANCE<br />AREA</strong></span></button>
        <button type="button" className="poster-photo poster-photo-right" onClick={() => onJump('pickup')} aria-label="进入待取车辆"><WorkbenchPicture className="poster-photo-media" position="82% center" /><span><small>02</small><strong>WORK<br />STATION</strong></span></button>
        <article className="poster-overview-card"><header><small>REAL-TIME WORKSHOP DATA</small><h2 id="poster-overview-title">TODAY'S<br />OVERVIEW</h2></header><dl>{dashboardRows.map((row) => <div key={row.en} data-accent={row.accent ? 'true' : 'false'}><dt><strong>{row.label}</strong><small>{row.en}</small></dt><dd>{row.value}</dd></div>)}</dl></article>
        <nav className="poster-operation-links" aria-label="业务模块跳转">{operationLinks.map((item) => <button type="button" key={item.id} onClick={() => onJump(item.id)}>{item.label}</button>)}</nav>
      </section>
      <footer className="poster-footer"><span>KEEP IMPROVING<br /><strong>MAKE IT COUNT</strong></span><i className="poster-globe" aria-hidden="true"><b /><b /></i><span>WORKSHOP<br /><strong>OPERATIONS</strong></span><ReleaseFooter /></footer>
    </main>
  </div>
}
