...
export default function PlatformAdminSetup({ token, onComplete }) {
  ...
  const options = directory.map((store) => ({ value: store.id, label: `${store.code} ${store.name}` }))
  ...
  return (
    <main className="registration-shell">
      <form className="registration-panel" onSubmit={submit} noValidate>
        <header><span>ONE-TIME PLATFORM SETUP</span><h1>初始化 CHU13</h1><p>CHU13 是门店目录和角色提权的唯一平台管理员。此链接只能成功执行一次。</p></header>
