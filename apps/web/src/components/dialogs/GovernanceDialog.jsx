...
export default function GovernanceDialog({ open, onClose, currentStoreId, onNotify }) {
  ...
  const decideJoin = async (item, approve) => {
    setBusy(true)
    try { const result = await decideJoinRequest(item.id, { ...emptyDecision, approve, expectedRevision: item.revision, reason: approve ? '门店管理员已批准' : '门店管理员已拒绝' }); onNotify?.(result.message); await refresh() } catch (requestError) { setError(requestError.message) } finally { setBusy(false) }
  }
  ...
  return (
    <AppDialog open={open} onClose={onClose} title="门店与权限治理" eyebrow="GOVERNANCE · 受审计审批" description="角色提权由 CHU13 审批；跨店调动与加入申请由目标门店的有效管理员审批（平台管理员兜底）。门店可以自行注册，首位注册人自动成为管理员。" className="governance-dialog">
      ...
      {data ? <div className="governance-stack">
        ...
        {data.joinRequests?.length ? <section className="governance-section"><h3>待审批加入</h3><ol className="governance-request-list">{data.joinRequests.map((item) => <li key={item.id}><div><strong>{item.userName || '申请人'}</strong><span>{item.storeCode} {item.storeName}</span><small>{item.usernameKey ? `账号 ${item.usernameKey} · ` : ''}申请于 {new Date(item.createdAt).toLocaleDateString('zh-CN')}</small></div><div className="governance-decisions"><button type="button" className="secondary-action" onClick={() => void decideJoin(item, false)} disabled={busy}>拒绝</button><button type="button" className="primary-action" onClick={() => void decideJoin(item, true)} disabled={busy}>批准</button></div></li>)}</ol></section> : null}
