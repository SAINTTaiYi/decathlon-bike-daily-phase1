...
export default function ShipHubSettingsDialog({ open, onClose, shiphub, onNotify, canManage = false }) {
  ...
  return (
    <AppDialog open={open} onClose={() => { if (!busy) onClose() }} title="Shiphub 连接" eyebrow="SINGLE-STORE SSO" description="填写本店的 Shiphub 门店账号完成连接。凭据以 AES-256-GCM 加密保存在 Cloudflare 本店专属行，自动重连与同步只使用本店身份，绝不落日志或业务库。">
      ...
      {fixture ? <p className="dialog-error" role="note">Preview 使用仓库内人工构造的 fixture：表单仅供界面预览，连接按钮停用，不会访问真实 Shiphub。</p> : null}
      {!fixture && status === 'connected' ? <button type="button" className="dialog-action" onClick={disconnect} disabled={busy}><span><strong>{busy ? '正在断开…' : '断开 Shiphub'}</strong><small>删除当前门店的本地 refresh token，不影响手工台账。</small></span></button> : null}
      <button type="button" className="dialog-action" onClick={connect} disabled={busy || fixture}><span><strong>{fixture ? '连接 Shiphub（Preview 不可用）' : busy ? '正在验证并连接…' : status === 'connected' ? '更新账号并重连' : '连接 Shiphub'}</strong><small>{fixture ? 'Preview fixture 不启用 SSO；此按钮在正式环境可用。' : effectiveShowStoreLogin ? '使用上方本店账号完成 IdP 登录与授权（OAuth2 authorization code + PKCE）。' : '使用已保存的本店账号自动完成 IdP 登录与授权。'}</small></span></button>
