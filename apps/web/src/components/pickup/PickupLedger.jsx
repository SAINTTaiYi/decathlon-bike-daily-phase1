...
function PickupCard({ record, index, expanded, density, query, closedAt, pickupError, primaryProcessing, primaryActionBusy, pickupPixelFill, repairMode = false, handoverMode = false, onToggle, onEdit, onRemove, onHistory, onPickup, onRepairComplete, onNotificationChange, onPickupPixelFillComplete }) {
  ...
  return <div ref={frameRef} className="pickup-card-frame" data-delete-open={deleteOpen ? 'true' : undefined} data-expanded={expanded ? 'true' : undefined}>
    ...
    <article className="pickup-card" data-density={density} data-expanded={expanded ? 'true' : undefined} data-error={pickupError ? 'true' : undefined} data-processing={pickupPixelFill ? 'true' : undefined} onPointerDown={onPointerDown} onPointerUp={onPointerUp} onPointerCancel={() => { startXRef.current = null }}>
      ...
      <div ref={detailRef} className="pickup-card-detail" id={`pickup-detail-${record.id}`}>
        ...
        {handoverMode ? <section className="pickup-detail-wide"><h4>STATUS <span>/ 当前状态</span></h4><p>{resultLabel}</p></section> : <section><h4>{repairMode ? 'SERVICE' : 'ORIGIN'} <span>/ {repairMode ? '维修' : '来源'}</span></h4><dl><div><dt>{repairMode ? '维修类型' : '待取来源'}</dt><dd>{repairMode ? (record.repairType || '未指定') : `${pickupSourceLabel(record)}${platform ? ` · ${platform}` : ''}`}</dd></div><div><dt>{repairMode ? '当前状态' : '业务结果'}</dt><dd>{resultLabel}</dd></div></dl></section>}
