...
function PickupCard({ record, index, expanded, density, query, closedAt, pickupError, primaryProcessing, primaryActionBusy, pickupPixelFill, repairMode = false, handoverMode = false, handoverStampEntering = false, members = [], onToggle, onEdit, onRemove, onHistory, onPickup, onRepairComplete, onHandoverComplete, onNotificationChange, onAssignClick, onPickupPixelFillComplete }) {
  ...
  const contact = repairPickup || handoverMode ? { contactType: record.contactType === 'member' ? 'member' : 'phone', contactValue: String(record.contactValue || '').trim() } : decodePickupContact(record)
...
export default function PickupLedger({ records = [], closedAt, onAdd, onEdit, onRemove, onHistory, onPickup, onRepairComplete, onHandoverComplete, onPickupNotificationChange, pickupErrors = {}, primaryProcessingId = '', primaryActionBusy = false, pickupPixelFillId = '', onPickupPixelFillComplete, repairMode = false, handoverMode = false, repairPixelDissolveId = '', onRepairPixelDissolveComplete, members = [], onAssign = null, shiphub = null }) {
  ...
  const queueControlsRendered = searchTarget && toolsTarget
    ? <>{createPortal(searchField, searchTarget)}{createPortal(<div className="pickup-tools-area"><div className="pickup-tool-row">{toolButtons}</div>{appliedFilterRow}</div>, toolsTarget)}</>
    : queueControls
