...
export default function RegistrationWizard({ onBack, onComplete }) {
  ...
  const sendOtp = async (event) => {
    ...
    if (!form.storeCode.trim() || !form.storeName.trim()) return setError('请填写门店编码和门店名称。')
        ...
        {step === 0 ? (
          <form className="data-form" onSubmit={sendOtp} noValidate>
            <div className="registration-directory-grid">
              <label className="field-row"><span>门店编码</span><input required autoComplete="organization" maxLength="32" value={form.storeCode} onChange={(event) => { clearFeedback(); set('storeCode', event.target.value) }} placeholder="例如 1299" aria-describedby="registration-store-help" /></label>
            ...
            <small id="registration-store-help" className="registration-profile-help">门店编码必须是公司内部使用的唯一编码。已有门店会直接加入，新的编码会在完成注册后激活门店。</small>
