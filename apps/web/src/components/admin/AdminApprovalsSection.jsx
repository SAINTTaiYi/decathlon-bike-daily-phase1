...
import { formatDeadline, formatStamp } from './admin-format.js'
...
const requestStatusLabels = { pending: '待审批', approved: '已批准', rejected: '已拒绝', cancelled: '已撤回' }
const requestStatusTones = { pending: 'pending', approved: 'active', rejected: 'danger', cancelled: 'disabled' }
...
export default function AdminApprovalsSection({ shared, directory = [] }) {
  ...
  const pendingStores = useMemo(() => (directory || []).filter((store) => store.status === 'pending'), [directory])
  ...
  return <section className="admin-panel"><header className="admin-panel-head"><h2>审批</h2><small>APPROVALS · 受审计决策</small></header>
    ...
    <div id="admin-approval-panel" role="tabpanel" aria-labelledby={`admin-tab-${tab}`} className="admin-approval-list">
      ...
      {currentItems.map((item) => { const isStore = tab === 'store'; const isRole = tab === 'role'; return <article key={item.id} className="admin-approval-row"><label className="admin-approval-check"><input type="checkbox" checked={Boolean(selected[item.id])} onChange={() => toggleSelect(item.id)} aria-label={`选择 ${item.userName || item.name || item.id}`} /></label><div className="admin-approval-identity"><strong>{isStore ? `${item.code} ${item.name}` : item.userName || '申请人'}</strong>{isStore ? <span className="admin-approval-place">{item.code} {item.name}</span> : isRole ? <span className="admin-approval-place">{item.storeCode} {item.storeName}</span> : <span className="admin-approval-move"><span className="admin-approval-move-from">{item.sourceStoreCode} {item.sourceStoreName}</span><span className="admin-approval-move-arrow" aria-hidden="true">→</span><span className="admin-approval-move-to">{item.targetStoreCode} {item.targetStoreName}</span></span>}{isRole ? <span className="admin-approval-role-shift">{shared.roleLabels[item.fromRole] || item.fromRole}<span aria-hidden="true"> → </span><strong>{shared.roleLabels[item.targetRole] || item.targetRole}</strong></span> : null}{isStore ? <span className="admin-status-tag" data-status="pending">待审核</span> : item.status && item.status !== 'pending' ? <span className="admin-status-tag" data-status={requestStatusTones[item.status] || 'disabled'}>{requestStatusLabels[item.status] || item.status}</span> : null}</div><div className="admin-approval-detail"><p>{isStore ? '新门店申请，批准后门店生效并开放注册。' : item.reason || <span className="admin-approval-no-reason">申请人未填写理由</span>}</p><ApprovalMeta item={item} isStore={isStore} />{item.decisionReason ? <p className="admin-approval-decision">审批意见：{item.decisionReason}</p> : null}</div>{(group === 'pending' || isStore) ? <div className="admin-approval-actions"><button type="button" className="secondary-action" disabled={busy} onClick={() => setDecision({ item, kind: tab, approve: false })}>拒绝</button><button type="button" className="primary-action" disabled={busy} onClick={() => setDecision({ item, kind: tab, approve: true })}>批准</button></div> : null}</article> })}
...
function ApprovalMeta({ item, isStore }) {
  const created = formatStamp(item.createdAt)
  const deadline = isStore ? null : formatDeadline(item.expiresAt)
  return <span className="admin-approval-meta">
    <span className="admin-approval-meta-item"><span className="admin-approval-meta-label">提交</span><time dateTime={item.createdAt} title={created.full}>{created.day} {created.clock}</time></span>
    {!isStore && Number.isFinite(Number(item.revision)) ? <span className="admin-approval-meta-item"><span className="admin-approval-meta-label">修订</span><span>{item.revision}</span></span> : null}
    {deadline ? <span className="admin-approval-meta-item" data-tone={deadline.tone}><span className="admin-approval-meta-label">截止</span><time dateTime={item.expiresAt} title={deadline.full}>{deadline.text}</time></span> : null}
  </span>
}
