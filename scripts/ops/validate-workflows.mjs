...
for (const sourceName of ['deploy-cloudflare-staging.yml', 'deploy-cloudflare-preview.yml', 'deploy-production.yml']) {
  assert(!/(?:RAILWAY|R2_|EDGEONE_SITE_URL|MIGRATION_DATABASE_URL)/u.test(workflows[sourceName] || ''), `${sourceName}: legacy provider variables are forbidden`)
}
...
assert(/= "6"/u.test(ci), 'ci: all six PostgreSQL compatibility migrations must be recorded')
...
assert(/CLOUDFLARE_ACCOUNT_ID: \$\{\{ vars\.CLOUDFLARE_ACCOUNT_ID \}\}/u.test(staging), 'cloudflare staging: account ID must come from the staging Environment variable')
assert(/STAGING_BASE_URL: \$\{\{ vars\.STAGING_BASE_URL \}\}/u.test(staging), 'cloudflare staging: site URL must come from the staging Environment variable')
assert(/git rev-parse "origin\/\$BRANCH"/u.test(staging), 'cloudflare staging: release SHA must equal selected remote branch head')
...
assert(/PREVIEW_BASE_URL: \$\{\{ vars\.PREVIEW_BASE_URL \}\}/u.test(preview), 'cloudflare preview: site URL must come from the preview Environment variable')
...
assert(/pnpm check:workflows && pnpm test && pnpm typecheck && pnpm build/u.test(preview), 'cloudflare preview: full validation is required')
...
assert(/CLOUDFLARE_ACCOUNT_ID: \$\{\{ vars\.CLOUDFLARE_ACCOUNT_ID \}\}/u.test(production), 'production: account ID must come from the production Environment variable')
...
assert(/bike-ops-production/u.test(production), 'production: Worker name must remain pinned')
...
includesInOrder(production, [
  ...
  'Deploy the Production Worker with Static Assets and D1',
  'Verify the deployed Production API, release identity, and Web shell'
], 'production')

assert(/0011_directory_guangxi_cities/u.test(schemaVersion), 'schema version: latest D1 migration must be reflected in runtime metadata')
assert(migrationNames.at(-1)?.startsWith('0011_'), 'schema version: latest committed D1 migration must be 0011')
...
console.log(JSON.stringify({ ok: true, yamlParser, workflows: workflowNames, policies: 96 }, null, 2))
