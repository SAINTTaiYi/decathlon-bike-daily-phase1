...
assert(workflowNames.join(',') === 'ci.yml,deploy-cloudflare-preview.yml,deploy-cloudflare-staging.yml,deploy-production.yml,deploy-staging.yml,onboard-workshop-skin-staging.yml,ops-pause-non-chu13-users.yml,ops-release-v650-20260903.yml', 'workflows: expected workflow set is present')
for (const [name, source] of Object.entries(workflows)) {
...
assert(latestMigration.startsWith('0034_'), 'schema version: latest committed D1 migration must be 0034 shiphub sync runs started index')
